...
function sourceBadge(source: CropRecordRow['source_type']) {
  if (source === 'developer') {
    return <span className="fv-badge text-[10px] bg-primary/10 text-primary border-primary/30">Developer note</span>;
  }
  return <span className="fv-badge text-[10px] bg-emerald-50 text-emerald-700 border-emerald-300">Company note</span>;
}
...
export default function AdminCropRecordsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      await createNote.mutateAsync({ title: t, content: c });
        ...
        <TabsContent value="notes" className="space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="fv-card p-8 text-center text-muted-foreground text-sm">
              {search ? 'No notes match your search.' : 'No notes yet. Add your first note for this crop.'}
            </div>
          ) : (
              ...
              <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {filteredRows.map((record) => (
                  <NoteListItem
                    ...
                    record={record}
                    onOpen={() => navigate(`/records/view/${record.record_id}`)}
                  />
                ))}
              </div>
              {totalPages > 1 && (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={createNote.isLoading}>
              {createNote.isLoading && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
