...
import { Plus, Search, Download, Calendar as CalendarIcon, Receipt, Loader2, Pencil } from 'lucide-react';
...
function toEditModalTarget(e: ExpenseWithSyncState): AddExpenseEditTarget {
  return {
    id: e.id,
...
function expenseCanBeEditedInModal(e: ExpenseWithSyncState): boolean {
  if (e.autoGenerated) return false;
  if (e.meta?.source === TOMATO_HARVEST_PICKER_LABOUR_EXPENSE_SOURCE) return false;
  if (e.meta?.source === 'harvest_wallet_picker_payment') return false;
  if (e.meta?.autoGenerated) return false;
  return true;
}
  ...
  const { data: allExpenses = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (typeof navigator !== 'undefined' && navigator.onLine) {
        try {
          await ExpenseService.pullRemote(companyId, {
            farmId: (selectedFarmId || activeProject?.farmId) ?? null,
            projectId: activeProject?.id ?? null,
          });
        } catch {
          // Local store still used when pull fails
        }
      }
    ...
    <div className="space-y-6 animate-fade-in" data-tour="staff-expenses-root">
      ...
      {(canCreateExpense || canEditExpense) && (
        <AddExpenseModal
          ...
          projectId={activeProject?.id ?? null}
          createdBy={user?.id ?? null}
          editingExpense={expenseToEdit ? toEditModalTarget(expenseToEdit) : null}
          onSaved={async () => {
        ...
        <DialogContent className="max-w-md">
          <DialogHeader className="space-y-2">
            <div className="flex items-start justify-between gap-2">
              ...
              {canEditExpense && expenseDetail && expenseCanBeEditedInModal(expenseDetail) ? (
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  ...
                  onClick={() => {
                    setExpenseToEdit(expenseDetail);
