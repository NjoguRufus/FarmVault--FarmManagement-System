...
export default function BrokerHarvestDetailsPage() {
  ...
  const harvest = useMemo(
    ...
    [allHarvests, harvestId, brokerId],
  );
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <button
          ...
          onClick={() => navigate('/broker/harvest-sales')}
        >
          <ChevronLeft className="h-4 w-4" />
          Back
        </button>
        <p className="text-muted-foreground">Harvest not found or not allocated to you.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="fv-card p-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {formatDate(harvest.date)} • {harvest.marketName ?? 'Market'}
        </p>
      </div>
