/**
 * FarmVault SEO constants – Kenya-first, scalable for East Africa.
 */

export const SEO_BASE_URL = typeof window !== "undefined"
  ? `${window.location.protocol}//${window.location.host}`
  : "https://farmvault.co.ke";

export const DEFAULT_SITE_NAME = "FarmVault";
export const DEFAULT_TITLE = "FarmVault | Farm Management Software Kenya";
export const DEFAULT_DESCRIPTION =
  "FarmVault helps Kenyan farmers manage crops, budgets, inventory, expenses and harvest logistics in one intelligent farm management system.";
export const DEFAULT_OG_IMAGE = "/Logo/FarmVault_Logo dark mode.png";
export const TWITTER_HANDLE = "@FarmVault";
export const LOCALE = "en_KE";

export const TITLE_MAX_LENGTH = 60;
export const DESCRIPTION_MIN_LENGTH = 150;
export const DESCRIPTION_MAX_LENGTH = 160;
