-- Fix: activate_subscription_from_mpesa_stk now returns the uuid of the inserted
-- subscription_payment row so the STK callback can pass it to billing-receipt-issue
-- for PDF generation and receipt email delivery.
-- Previously the function returned void, so subPayId was always null in the callback
-- and billing-receipt-issue was never invoked.
...
begin
  ...
  -- Already activated — return the existing subscription_payment id if we can find it
  if coalesce(v_payment.subscription_activated, false) then
    select id into v_new_payment_id
    from public.subscription_payments
    where company_id = v_payment.company_id::text
  ...
  -- Upsert company_subscriptions: override trial, set active
  ...
  -- Sync canonical fields on core.companies: clear trial, set paid plan + active_until
  ...
  -- Insert approved payment record and capture the new row id
  ...
  -- Mark STK payment row as fully processed
