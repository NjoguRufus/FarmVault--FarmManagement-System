-- Fix subscription status computation inputs
-- Add canonical payment_confirmed/trial_ends_at/active_until fields to core.companies
-- and expose them via list_companies_v2 so UI can compute status without plan.
...
-- Ensure list_companies includes these canonical fields (when present).
...
begin
  ...
  -- Prefer canonical company.active_until if present, otherwise subscription row.
