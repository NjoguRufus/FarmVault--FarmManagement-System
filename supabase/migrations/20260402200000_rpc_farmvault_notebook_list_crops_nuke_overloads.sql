-- PostgREST still returns 22P02 invalid uuid "" when ANY stale overload (e.g. p_company_id uuid)
-- exists or the schema cache is ambiguous. Regex-based drops can miss signatures.
-- Nuke every public.rpc_farmvault_notebook_list_crops overload, then expose exactly one TEXT arg.
-- No DEFAULT on p_company_id (avoids ambiguous {} resolution vs uuid overloads).
