-- list_company_payments: include failed M-Pesa STK rows (same as developer Payments tab).
-- Replaces function from 20260406162000; drop + create required.
  ...
  if admin.is_developer() then
    ...
    from (
      ...
      select
        ...
        mpf.company_id::text,
        case
          when lower(btrim(coalesce(mpf.plan, ''))) like '%pro%' then 'pro'
          when lower(btrim(coalesce(mpf.plan, ''))) = 'basic'   then 'basic'
          else 'pro'
        end,
        coalesce(mpf.amount, 0),
        'failed'::text,
        'mpesa_stk'::text,
        case
          when lower(btrim(coalesce(mpf.billing_cycle, ''))) in ('monthly', 'seasonal', 'annual')
            then lower(btrim(mpf.billing_cycle))
          else 'monthly'
        end,
        'KES'::text,
        'mpesa_stk'::text,
        null::text,
        nullif(trim(mpf.phone), ''),
        nullif(trim(mpf.mpesa_receipt), ''),
        coalesce(nullif(trim(mpf.result_desc), ''), 'M-Pesa STK (failed)')::text,
        mpf.created_at,
        mpf.created_at,
        null::timestamptz,
        'mpesa_stk'::text
      from public.mpesa_payments mpf
      where mpf.company_id = _company_id
        and (
          upper(trim(coalesce(mpf.status, ''))) in ('FAILED', 'FAILURE')
          or (mpf.result_code is not null and mpf.result_code <> 0)
        )
        and not (
          (mpf.result_code is not null and mpf.result_code = 0)
          or upper(trim(coalesce(mpf.status, ''))) in ('SUCCESS', 'COMPLETED')
        )
    ) u
