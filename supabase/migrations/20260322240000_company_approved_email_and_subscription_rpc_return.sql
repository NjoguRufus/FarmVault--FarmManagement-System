-- 1) Track workspace-ready email so we never spam on duplicate approval clicks or edits.
-- 2) set_company_subscription_state returns jsonb with workspace_ready_email when pending_approval -> trial/active (approve/activate).

begin;

alter table public.company_subscriptions
  add column if not exists company_approved_email_sent_at timestamptz;

comment on column public.company_subscriptions.company_approved_email_sent_at is
  'When set, the company admin was sent the FarmVault workspace-ready (company approved) email.';

-- Return type change (void -> jsonb) requires drop in Postgres.
