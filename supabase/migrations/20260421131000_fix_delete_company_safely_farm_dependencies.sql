-- Fix: developer company delete must handle farm-linked dependencies.
-- When core.companies is deleted it cascades to projects.farms, but several tables
-- reference farms with ON DELETE RESTRICT (e.g. ops.work_cards, finance.expenses).
-- This migration updates developer.delete_company_safely to clean those dependencies first.
...
begin
  ...
  -- ---------------------------------------------------------------------------
  -- Farm-linked dependencies (MUST be deleted before company -> farms cascade).
  -- ---------------------------------------------------------------------------
