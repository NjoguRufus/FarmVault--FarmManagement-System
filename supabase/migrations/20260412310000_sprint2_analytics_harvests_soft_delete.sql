-- Sprint 2: analytics RPCs — exclude soft-deleted harvest.harvests rows (deleted_at) in addition to
-- Phase 2 project/expense/collection rules. Replaces 20260412180000 definitions after harvests.deleted_at exists.
...
begin
  ...
  if use_hct and use_hct_created_at and (use_hct_total_revenue or use_hct_total_gross) then
    return query
      ...
      join harvest.harvest_collections hc on hc.id = hct.collection_id and hc.deleted_at is null
      join projects.projects p on p.id = hc.project_id and p.deleted_at is null
  ...
  return query
    ...
    join projects.projects p on p.id = h.project_id and p.deleted_at is null
  ...
  if use_hct and use_hct_crop and (use_hct_total_revenue or use_hct_total_gross) then
    return query
      ...
      exp as (
        ...
        where e.company_id = p_company_id
          and e.deleted_at is null
          and (e.project_id is null or p.deleted_at is null)
        group by 1
      )
