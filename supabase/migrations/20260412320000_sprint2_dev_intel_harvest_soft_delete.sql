-- Sprint 2: developer company intelligence — exclude soft-deleted harvest.harvests (deleted_at).
-- Replaces 20260412170000 after harvests.deleted_at exists.
...
begin
  ...
  v_metrics := jsonb_build_object(
    'projects_total', (select count(*)::bigint from projects.projects p where p.company_id = p_company_id and p.deleted_at is null),
    'users_total', v_users_count,
    'employees_total', v_employees_count,
    'harvest_records_total', (select count(*)::bigint from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id and h.deleted_at is null),
    'harvest_quantity_total', coalesce((
      select sum(h.quantity)::numeric from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id and h.deleted_at is null
    ), 0),
    'harvest_revenue_total', coalesce((
      select sum(h.quantity * coalesce(h.price_per_unit, 0))::numeric
      from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id and h.deleted_at is null
    ), 0),
    'expenses_total', coalesce((
      select sum(e.amount)::numeric from finance.expenses e where e.company_id = p_company_id and e.deleted_at is null
    ), 0),
    'expense_count', (select count(*)::bigint from finance.expenses e where e.company_id = p_company_id and e.deleted_at is null),
    ...
    'collections_total', (select count(*)::bigint from harvest.harvest_collections hc where hc.company_id = p_company_id and hc.deleted_at is null),
  ...
  select greatest(
    ...
    (select max(h.created_at) from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id and h.deleted_at is null),
  ...
  from (
    select
      ...
      (select count(*)::bigint from harvest.harvests hx where hx.project_id = p.id and hx.deleted_at is null) as harvest_count,
      (select coalesce(sum(ex.amount), 0)::numeric from finance.expenses ex where ex.project_id = p.id and ex.deleted_at is null) as actual_spend,
  ...
  from (
    ...
    where h.company_id = p_company_id and h.deleted_at is null and (pr.id is null or pr.deleted_at is null)
  ...
  || format($f$
      ...
      from harvest.harvests h
      left join projects.projects pr on pr.id = h.project_id
      where h.company_id = $1 and h.deleted_at is null and (pr.id is null or pr.deleted_at is null)
    union all
      select
        'harvest_collection',
        'Harvest collection',
