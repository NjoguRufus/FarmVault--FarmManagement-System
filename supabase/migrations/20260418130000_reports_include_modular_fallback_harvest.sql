-- Reports: include modular fallback harvest sessions + tomato sessions in revenue/profit aggregates.

begin;

-- -----------------------------------------------------------------------------
-- analytics_monthly_revenue(company) — extend to include tomato + fallback sessions
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Fallback: legacy harvest.harvests + tomato + fallback sessions.
  return query
    with base as (
...
-- -----------------------------------------------------------------------------
-- analytics_report_detail_rows(company) — include tomato + fallback revenue by date/crop
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Fallback: compute revenue from harvest.harvests + tomato + fallback.
  return query execute format(
    $q$
      with revenue as (
