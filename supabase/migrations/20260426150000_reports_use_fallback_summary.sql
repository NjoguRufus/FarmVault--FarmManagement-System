-- Reports/analytics: compute fallback harvest revenue/expenses from raw data (no stored totals).
-- Updates:
-- - public.analytics_monthly_revenue: fallback revenue from harvest.get_fallback_session_summary
-- - public.analytics_report_detail_rows: fallback revenue from summary + fallback market expenses in expenses CTE
...
declare
  ...
  use_fb boolean := public._table_exists('harvest', 'fallback_harvest_sessions')
    and public._table_exists('harvest', 'fallback_harvest_units');
begin
      ...
      return query execute $sql$
        with base as (
          ...
          select date_trunc('month', fs.created_at)::date as month,
                 coalesce(sum(x.revenue_total), 0)::numeric as revenue
          from harvest.fallback_harvest_sessions fs
          join lateral harvest.get_fallback_session_summary(fs.id) x on true
...
begin
    ...
    if use_fb then
      return query execute format(
        ...
        exp as (
          ...
          select (fs.created_at at time zone 'utc')::date as date,
                 nullif(trim(p.%2$I::text), '') as crop,
                 coalesce(sum(m.amount), 0)::numeric as expenses
          from harvest.fallback_harvest_sessions fs
          join projects.projects p on p.id = fs.project_id
          join harvest.fallback_market_dispatches md on md.harvest_session_id = fs.id and md.company_id = fs.company_id
          join harvest.fallback_market_expense_lines m on m.market_dispatch_id = md.id and m.company_id = fs.company_id
          where fs.company_id = $1
          group by 1, 2
        ),
        y as (
        ...
        select coalesce(revenue.date, exp.date, y.date) as date,
               ...
               coalesce(sum(exp.expenses), 0)::numeric as expenses,
               (coalesce(sum(revenue.revenue), 0) - coalesce(sum(exp.expenses), 0))::numeric as profit,
      ...
      -- No fallback table: keep legacy path
      return query execute format(
        $q$
        with revenue as (
          select (hct.created_at at time zone 'utc')::date as date,
  ...
  -- Non-HCT fallback: keep existing behavior (not expanded here)
  return;
end;
$$;
