-- PostgREST can still bind rpc_farmvault_notebook_list_crops to a uuid-typed overload (or coerce "" to uuid),
-- producing: invalid input syntax for type uuid: "".
-- Drop every overload except the single-argument TEXT form; recreate without DEFAULT so {} cannot be resolved ambiguously.
  ...
  for r in
    ...
    where n.nspname = 'public'
      ...
      and (
        ...
        or nullif(trim(pg_get_function_identity_arguments(p.oid)), '') is null
      )
  loop
...
-- PG forbids removing parameter defaults via CREATE OR REPLACE; must drop the (text) overload first.
-- CASCADE drops dependent list_company_record_crops; we recreate it below.
...
begin
  ...
  if v_company_id is null then
    v_company_id := nullif(trim((select public.current_company_id()::text)), '');
  end if;
