-- Analytics RPCs: only reference deleted_at when the column exists (avoids errors if Sprint 2
-- harvests migration not applied yet, or older DBs). Uses public._column_exists (from reports migrations).
-- Superseded for greenfield deploys by 20260412350000_soft_delete_standardize_core_entities.sql (re-applies
-- same function bodies + adds deleted_at columns). Safe to keep both: idempotent CREATE OR REPLACE.
