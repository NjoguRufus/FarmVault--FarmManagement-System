...
begin
  ...
  v_metrics := jsonb_build_object(
    ...
    'harvest_records_total', (select count(*)::bigint from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id),
    'harvest_quantity_total', coalesce((
      select sum(h.quantity)::numeric from harvest.harvests h join projects.projects p_hp on p_hp.id = h.project_id and p_hp.deleted_at is null where h.company_id = p_company_id
    ), 0),
