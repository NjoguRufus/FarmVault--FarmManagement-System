-- Run this in Supabase Dashboard → SQL Editor to fix RLS blocking company creation.
-- Enables RLS and adds permissive policies so onboarding can create company, add membership, update profile.

ALTER TABLE public.companies ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.company_members ENABLE ROW LEVEL SECURITY;

-- Companies: drop existing, then create
DO $$
DECLARE pol RECORD;
BEGIN
...
-- Profiles: drop existing, then create
DO $$
DECLARE pol RECORD;
BEGIN
...
CREATE POLICY "users can read profile" ON public.profiles FOR SELECT USING (true);
CREATE POLICY "users can update profile" ON public.profiles FOR UPDATE USING (true) WITH CHECK (true);

-- Company members: drop existing, then create
DO $$
DECLARE pol RECORD;
BEGIN
...
CREATE POLICY "users can add membership" ON public.company_members FOR INSERT WITH CHECK (true);
CREATE POLICY "users can read memberships" ON public.company_members FOR SELECT USING (true);
